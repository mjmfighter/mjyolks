...
# If GITHUB_URL and GITHUB_PRIVATE_KEY are set, we'll use them to clone the repository to /home/container/repo (current user is container)
if [ -n "$GITHUB_URL" ] && [ -n "$GITHUB_PRIVATE_KEY" ]; then
  ...
  # Clone the repository to /home/container/repo using GITHUB_BRANCH if it's set
  if [ -n "$GITHUB_BRANCH" ]; then
    git clone --single-branch --branch "$GITHUB_BRANCH" "$GITHUB_URL" /home/container/repo
  else
    git clone "$GITHUB_URL" /home/container/repo
  fi
  cd /home/container/repo || exit 1
